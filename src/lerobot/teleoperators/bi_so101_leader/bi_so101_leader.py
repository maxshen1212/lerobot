...
class BiSO101Leader(Teleoperator):
    ...
    Identical to ``BiSO100Leader`` except that it wraps ``SO101Leader`` instead of ``SO100Leader``.
    The two differ only in ``calibrate()``: ``SO100Leader`` skips ``wrist_roll`` in the
    range-of-motion sweep and hard-codes its range to the full encoder span (0..4095), while
    ``SO101Leader`` sweeps every joint. Under ``RANGE_M100_100`` normalization that span is the
    scale, so a hard-coded one leaves a gain error on ``wrist_roll``.

    Calibration file names are unchanged, so this is a drop-in replacement for
    ``bi_so100_leader`` in an existing setup. See ``BiSO101Follower`` for the fuller note.
    """
