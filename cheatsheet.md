...
export CUDA_VISIBLE_DEVICES=1,5,6
echo $CUDA_VISIBLE_DEVICES
...
accelerate launch \
  ...
  $(which lerobot-train) \
  --dataset.repo_id=lbm_sim/ego_BimanualPlaceAppleFromBowlOnCuttingBoard \
  --dataset.root=/data/maxshen/lerobot_training_data_v3/ego_BimanualPlaceAppleFromBowlOnCuttingBoard_lang \
  --policy.type=multi_task_dit \
  --policy.repo_id=CHIH-HAN/tri-DiT-BimanualPlaceAppleFromBowlOnCuttingBoard \
  --policy.push_to_hub=true \
  --output_dir="./outputs/multitask_dit_tri"
...
#########################################
CUDA_VISIBLE_DEVICES=1,4,5,6 accelerate launch \
  --multi_gpu \
  --num_processes=4 \
  --main_process_port 0 \
  $(which lerobot-train) \
  ...
  --output_dir="./outputs/multitask_dit_libero2" \
  ...
  --eval.batch_size=1 \
  --eval.n_episodes=10 \
  --policy.path=/data/maxshen/lerobot/outputs/multitask_dit_libero/checkpoints/030000/pretrained_model \
  --policy.n_action_steps=24 \
  --env.max_parallel_tasks=2

  python /data/maxshen/lerobot/run_eval.py \
  --output_dir=./eval_logs2/ \
  --env.type=libero \
  --env.task=libero_spatial \
  --eval.batch_size=1 \
  --eval.n_episodes=1 \
  --policy.path=/data/maxshen/lerobot/outputs/multitask_dit_libero/checkpoints/004000/pretrained_model \
  --policy.n_action_steps=24 \
